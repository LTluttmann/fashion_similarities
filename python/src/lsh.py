...
class LSH:
    ...
    def __init__(self, data, num_total_hashes, rows_per_band, dist_metric="euclidean", bucket_width=None):
        ...
        self.bucket_width = bucket_width or self.choose_bucket_width() if dist_metric == "euclidean" else None
    ...
    def choose_bucket_width(self):
        ...
        for _i in range(500):
            ...
            dists.append(self.calc_dist_euclidean(*sample_points))
    ...
    def get_near_duplicates(self, query: list, num_duplicates: int = None, add_query_to_db: bool = False):
        ...
        candidates = []
        for i, table in enumerate(self.hash_tables):
            hash = self.get_hash_euclidean(query, i)
            candidates.extend(table.get(hash, []))
        if add_query_to_db:
            self.extend_hash_tables(list(query))
        candidates = set(candidates)
        candidates = [(candidate, LSH.calc_dist_euclidean(query, self.data[candidate])) for candidate in candidates]
